  ...
  try {
    ...
    for (const obj of response.data) {
      ...
      if (result.found === true) {
        result.mongoDbId = obj.id;

        break;
      }
    }
    // alert("affichage des donnees de response");
    // alert(response.data);
...
export default function NewAlarms() {
  console.log("NewAlarms component rendered");
  ...
  const handleNavigateToEditAlarm = async (row: GridRowModel) => {
    console.log("handleNavigateToEditAlarm");
    // params.row.mongoDbId
    setSelectedRow(row);
    console.log("affichage de GridRowModel");
    console.log(row);
    console.log("affichage de row");
    console.log(selectedRow);
    if (selectedRow) {
      console.log("in the if handleNavigateToEditAlarm");
      ...
      setNewData(updatedRow);
      console.log(newData);
      console.log(newData);
      try {
        const response = await axios.put(
          `http://localhost:8080/api/${id}/newdata`,
          newData
        );
        // setMessage(
        //   `Successfully updated document with ID: ${response.data.id}`
        // );
      } catch (error) {
        // setMessage("Error updating document: " + (error as Error).message);
      }
      // const data = JSON.stringify(updatedRow);
      // const dataToSend = `{
      //   "newData":
      ...
      // }`;
      // try {
      //   const response = await fetch("http://localhost:8080/api/alarm", {
      //     method: "PUT",
      //     headers: {
  ...
  const getRowsCols = async (
    ...
    const updatedColumns = [
      ...
      },
      {
        ...
        headerName: "Actions",
        renderCell: (params) =>
          params.row.existsInMongo ? (
            <>
              <IconButton
                color="primary"
                aria-label="View"
                onClick={() => handleNavigateToEditAlarm(params.row)} // Navigate on button click
              >
                <EditIcon />
              </IconButton>
            </>

// src/components/TicketEditTemplate.tsx
...
interface Field {
  ...
  slas?: Sla[];
  editor?: Editor;
  // for UI-only selected SLA in dropdown (not persisted)
...
const TicketEditTemplate: FC = () => {
  ...
  const addSla = () => {
    setSlas([...slas, { priorityLabel: "", sla: 0 }]);
  };
  ...
  const handleSlaSelectChange = (fieldId: string, selectedSla: Sla | null) => {
    setFields((prevFields) =>
      prevFields.map((field) =>
        field.id === fieldId ? { ...field, selectedSla } : field
      )
    );
  };
  ...
  // load fields with excel file headers
  useEffect(() => {
  ...
  const handleAddField = () => {
    ...
    const newField: Field = {
      ...
      dateRange:
        fieldType === "DateRange"
          ? {
              startName: startCustomName,
              endName: endCustomName,
              startDateTime: "",
              endDateTime: "",
            }
          : undefined,
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setSnackbarMessage("Ticket template updated successfully.");
      setSnackbarSeverity("success");
      setSnackbarOpen(true);
    } catch (err: any) {
      const msg =
        err?.response?.data?.message || "Failed to update ticket template";
      setError(msg);
          ...
          <Stack spacing={3}>
            ...
            <Box sx={{ mb: 1 }}>
              ...
              <Typography variant="subtitle1" color="text.secondary">
                Update template name, fields, SLA configuration and description.
              </Typography>
            </Box>
                    ...
                    >
                      {fields.length === 0 ? (
                        <Typography
                          ...
                          sx={{ p: 1 }}
                        >
                          No fields defined yet. Use the section below to add
                          fields.
                        </Typography>
                      ) : (
                        fields.map((field) => (
            ...
            {loadingExcelData ? (
              ...
              sheets.length > 0 && (
                ...
                >
                  ...
                  <FormControl sx={{ width: 220, mb: 2 }} variant="outlined">
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        selectSheet(sheets[e.target.value as number])
                      }

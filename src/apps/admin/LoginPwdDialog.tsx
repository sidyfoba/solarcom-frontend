...
const LoginPwdDialog: React.FC<LoginPwdDialogProps> = ({
  open,
  onClose,
  signUpRequest,
}) => {
  ...
  const handleLoginChange = (login: string) => {
    if (editedUser) {
      setEditedUser({ ...editedUser, login });
    }
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      formDataToSend.append("login", editedUser.login);
      formDataToSend.append("password", editedUser.password);
      const token = sessionStorage.getItem("jwt");
      await axios.put(
        `http://localhost:8080/api/us/userlogin/${editedUser.id}`,
        formDataToSend,
        {
  ...
  return (
    <>
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>
          Modifier Login et mode de passe de l'utilisateur
        </DialogTitle>
        <DialogContent>
          <Grid container spacing={2}>
            ...
            <Grid item xs={6}>
              <TextField
                ...
                error={loginError}
                helperText={loginError && "Le login ne peut pas être vide"}
                disabled={!isEditable}
              />
            </Grid>
            <Grid item xs={6}>
